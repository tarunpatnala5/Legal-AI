...
export default function TopNavbar() {
    ...
    return (
        <div className="h-16 bg-background border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 shadow-sm transition-colors duration-300">
            {/* Search Bar - Hidden on small mobile, visible on tablet+ */}
            <div className="hidden sm:flex items-center bg-slate-100 dark:bg-slate-900 px-4 py-2 rounded-lg w-full max-w-md border border-transparent focus-within:border-blue-500 transition-all mx-4">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="relative group">
                    ...
                    <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 transform origin-top-right">
                        <div className="p-4 border-b border-slate-200 dark:border-slate-800">
                            <h3 className="font-semibold text-slate-800 dark:text-white">Upcoming Events (24h)</h3>
                        </div>
                            ...
                            {notifications.length === 0 ? (
                                ...
                                notifications.map(n => (
                                    <div key={n.id} className="p-3 hover:bg-slate-50 dark:hover:bg-slate-800 border-b last:border-0 border-slate-100 dark:border-slate-800 transition">
                                        <div className="font-medium text-sm text-slate-800 dark:text-slate-200">{n.case_name}</div>
                ...
                <button className="flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-900 pl-2 pr-4 py-1.5 rounded-full transition border border-slate-200 dark:border-slate-800">
